...
def _solve_pnp(
    ...
    T_initial = T           #[mbs:221026] added for testing 
    ...
    assert not (cloud is None) #[mbs:221020] testing
    assert not (
        (R is None) ^ (T is None)
    ...
    if T_initial is not None:
        print("ERRO PNP", np.linalg.norm(T- T_initial) )        #[mbs:221026] testing
    return R, T
...
def solve_pnp(config, track, mask, cloud, R=None, T=None):
    ...
    :param config: config object. See config.py for more information
    :param track: 2 column matrix with 2D features
    :param mask: point cloud indexes for features in the track slice
    :param cloud: point cloud with N points as a ndarray with shape Nx3
    :param R: rotation matrix seed
    :param T: translation vector seed
    :param method: method to be used in the algorithm
    ...
    if config.use_iterative_pnp:
        ...
        R, T = _solve_pnp(                                                             #[mbs:221026] doesnt work either
