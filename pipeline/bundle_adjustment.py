...
def _prepare_optimization_input(cloud, Rs, Ts, tracks, masks):
    ...
    for index, (track, track_mask) in enumerate(zip(tracks, masks)):        #(1,( [[a,b],[c,d],[d,e], ...], [[f,g], [h,i], ]) [1,2,3,...])
...
def _objective_function(
    ...
    """
    camera_params = params[: n_cameras * 6].reshape((n_cameras, 6))     #[rx, ry, rz, tx, ty, tz] for each cam (per line)
    points_3d = params[n_cameras * 6 :].reshape((n_points, 3))          #[X,Y,Z] (per line)
...
def _bundle_adjustment_sparsity(
    ...
    m = camera_indices.size * 2         #[mbs:221015] why times 2?
    n = n_cameras * 6 + n_points * 3
    A = lil_matrix((m, n), dtype=int)
    print(A)
    i = np.arange(camera_indices.size)
    for s in range(6):
        A[2 * i, camera_indices * 6 + s] = 1        #P parameters (rotation  + translation vectors)
        A[2 * i + 1, camera_indices * 6 + s] = 1

    for s in range(3):
        A[2 * i, n_cameras * 6 + point_indices * 3 + s] = 1
        A[2 * i + 1, n_cameras * 6 + point_indices * 3 + s] = 1     #X parameters
    return A
...
def _optimize(
    ...
    x0 = np.hstack((camera_params.ravel(), points_3d.ravel()))      #large array with initial estimates  (x1,y1,z1,r1,)
    ...
    optim_res = least_squares(
        objective_function,
        x0,
        jac_sparsity=A, #[mbs221020: commented for testing]
